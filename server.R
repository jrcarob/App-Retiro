...
simulate_nav <- function(start_capital = 50000, annual_mean_return = 5.0,
...
plot_nav <- function(nav) {
  ...
  matplot(nav,
          ...
          xlab = 'Meses', ylab = 'Millones',
          main = 'Valor Proyectado del Capital Inicial')
  
  # plot % of scenarios that are still paying
  p.alive = 1 - rowSums(is.na(nav)) / ncol(nav)
  
  plot(100 * p.alive, las = 1, xlab = 'Meses', ylab = 'Porcentaje Pagado',
       main = 'Porcentaje de Escenarios de Pago', ylim=c(0,100))
  grid()
  ...
  plot(density(final.nav, from=0, to=max(final.nav)), las = 1, xlab = 'Capital Final',
       main = paste0('Distribución del Capital Final\n', 100 * p.alive[last.period], '% que se continúa pagando'))
  grid()
}
